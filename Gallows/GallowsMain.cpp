	...
	do {
		...
		do {
			...
			*(guesses + guessnum) = eneteringAGuess(guesses, guessnum);
			char changedWord[999] = { '\0' };
			strcpy(switchedword, switchLetter(changedWord,*(guesses + guessnum), words, switchedword, randomnumber, &hits));	//switch letter is a function in 'GallowsFunctions.cpp'															
